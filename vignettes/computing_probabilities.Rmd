...
This is the evaluation of the conditional probability using Bayes' Theorem

$$
P(A|B) = \frac{P(B|A)P(A)}{P(B)}
$$

In our case, the prbability of B is indendpent of A, thus, the equation simplifies to 
...
The first value, $q_{ij \pi_i}$ is $p_{ij}$ if $\pi_i = 1$ and $0$ otherwise. Thus, if the $i^{th}$ value of $\pi$ from the considered sample $\pi$ is $1$ (i.e. gene number $i$ is mutated), then the probability of sample $j$ having a mutation in gene $g_i$ (i.e.the $i^{th}$ row and $j^{th}$ column of the weight matrix $W$, $W[i,j]$). (**Note:** the paper is not really clear what $p_{ij}$ is, so I made an assumption here based on my understanding of their formula and the common use of $p$ for "probability".)

The function $F$ takes in several paramters. The first is $w_{\pi}(t)$ which is $t-1$ is the sum of all values in the binary vector $\pi$ is equal to $1$, and $t$ otherwise. If the sum of $\pi$ is $1$, this indicates that there is only one gene being considered in that step of the iteration, so the number of mutually exclusive events ($t = t_M$) is reduced by $1$ for $F$. This ensures the number of mutually exclusive events is held constant.

The next set of paramaters for $F$ are $y_{\pi}(x)$ for every row-sum (i.e. number of samples with a mutation in gene $g_i$) of genes in $M$. This simply reduces the row-sum by $1$ if the the index in $\pi$ is $1$. This holds the row-sums constant.

Finally, $j-1$ redueces the number of samples by $1$ to make room for the new one introduced by the outer sum as $\pi$.

In the end, the function $F$ is not well described here. **TODO**
