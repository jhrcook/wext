...
```{r bench_method1, cache=TRUE, eval=FALSE}
...
```{r bench_methodC, cache=FALSE}
bench::mark(bipartite_edge_swap3(bgr, N = 100),
            iterations = 50, filter_gc = FALSE) %>% 
  dplyr::select(-expression)
```
...
```{r bench_methodsnull, cache=FALSE}
# first method
...
```{r bench_methodsnull2, cache=FALSE}
# first method
bench::mark(bipartite_edge_swap(bgr, N = 1),
            iterations = 50, filter_gc = FALSE) %>% 
  dplyr::select(-expression)
